...
O joose facilita o trabalho com orientação a objeto no javascript. Fica mais óbvio, simples, clean, declarativo e aumenta a produtividade. O sistema de meta objetos do joose é multiparadigma, ou seja, ele suporta desenvolvimento voltado a classes, prototypos e tambem suporta herança de classes e extensão com roles.
...
=head2 Pré-requisitos

Antes de iniciar o desenvolvimento com Joose, obviamente você vai precisar carregar a lib antes. ex:

  <script type="text/javascript" src="./lib/joose.js"></script>

ou concatenar o código do Joose antes de criar as classes.

=head2 O que NÃO é Joose JS

Joose não é um manipulador de DOM. Ex. ele não faz o que o jquery faz.

Ele não trabalha no nível de markup. Mas pode ser usado em conjunto com jquery, dojo, angular, extjs, YUI, Prototype, Mootools and PureMVC.
...
Como é feito normalmente em js: 
  ...
  var Pessoa = function() {
    ...
    //sobrenome
    this.sobrenome    = null;
    this.setSobrenome = function ( val ) { 
       this.sobrenome = val;
    };
...
*** O escopo do "override" disponibiliza a variável: 

  this.SUPER

que nada mais é que a referência para o método original. Você pode usála caso julgue necessário. ex:
...
=head4 Construção da classe

Para executar comandos logo após construir / instanciar uma classe, você precisara trabalhar em cima do método initialize.

O método initialize é chamado automaticamente sempre que você instancia uma classe.

    after: {
        initialize: function( args ) {
            console.log( "a classe está pronta e disponível para uso" );
            // ... download de dados, modificar coisas, etc ...
        } 
    }
...
=head4 Multiplos coerce

Para rodar vários coerce, tem que retornar null e depois retornar o item de entrada por exemplo:
...
Um prototipo joose suporta as mesmas funcionalidades do Class-builder, mas com uma diferença:

Todas as instancias do prototipo recebem seus próprios meta-objetos. Isso significa que ao manipular uma instancia de um protótipo usando a meta interface, as alterações afetarão apenas a instância em questão.
...
E ao instanciar um segundo prototipo a partir de um outro, o novo protótipo recebera todas as alterações, ex: 

  var proto2 = new proto1.constructor()
...
=head3 Exemplo 1 - Carros e motores
  ...
  Class( "Lamborghini", {
    ...
    after: {
      setEngine: function () {
        console.log( "Configurada classe com motor: " + this.engine.meta._name );//ou this.toString()
      }
    }
  } )
...
e o uso:
  ...
  var jeepao = new        Jeep( { engine: new Estilo4x4() } ); //instancia um jeep com motor Estilo4x4. Mas eu poderia trocar o motor ;)
...
=head3 Exemplo 3 - Computador

Neste exemplo, vou instanciar uma classe "Computador" que recebe um json com especificações de computador.

As especificações tem vários detalhes diferentes ao apresentado anteriormente, pois o atributo gabinete é um objeto que tem vários outros itens tipo hdd, placa mãe, etc. e eles tambem serão instanciados para suas respectivas classes.

Primeiro vou definir os tipos, e classes. E na classe Computador vou indicar que cada atributo é do tipo respectivo, ex, teclado é do TipoTeclado, gabinete é do TipoGabinete, e dentro do gabinete, tem os atributos ex hdd que é do TipoHarddisk
  ...
  Type( "TipoGabinete" , {
    ...
    coerce : [
      ...
      {
        //Define Placa Mãe, deixei como exemplo para fazer vários coerce, veja que retorna null no final deste bloco
        from : Joose.Type.Any,
         via : function ( gabinete ) {
          if ( ! gabinete || !gabinete.placa_mae ) return null;
          if ( /object/ig.test( gabinete.placa_mae.constructor ) ) {
...
http://joose.github.io/Joose/doc/html/Joose.html
