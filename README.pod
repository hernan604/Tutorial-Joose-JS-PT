...
Pra falar bem grossamente, uma classe pode ser montada com várias roles. 
...
As roles não podem ser instanciadas, ou seja, não é possível fazer:
  ...
  var role_programador = new Programador_Role(); // Se Programador_Role for uma role, isso vai dar pau.

As roles são para ser utilizadas nas classes. Role em inglês quer dizer "papel", no sentido de atores por exemplo.. uma hora o ator exerce o papel de algo.
